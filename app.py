# app.py → VERSION FINALE ULTIME – 100% TA LOGIQUE + PDF TEXTE PARFAIT
...
# ReportLab pour PDF 100% texte sélectionnable
...
def create_pdf(df):
    ...
    col_widths = [80, 50, 50, 90, 65, 55, 140]
...
if st.button("Lancer le Scan", type="primary", use_container_width=True):
    with st.spinner("Scan en cours sur 124 timeframes..."):
        ...
        with ThreadPoolExecutor(max_workers=10) as exe:
            futures = {exe.submit(get_candles, i, c): (i, n) for i in INSTRUMENTS for n, c in TIMEFRAMES.items()}
        ...
        if results:
            ...
            st.success(f"Scan terminé – {len(df_res)} signaux détectés")
        else:
...
if "df" in st.session_state:
    ...
    with c2:
        fig, ax = plt.subplots(figsize=(14, len(df)*0.35))
        ax.axis('off')
