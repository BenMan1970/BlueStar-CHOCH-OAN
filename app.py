...
# ReportLab pour PDF texte sélectionnable (remplace FPDF)
...
def create_pdf(df):
    ...
    col_widths = [70, 50, 45, 80, 60, 45, 60, 60, 60, 100]  # Calibré pour éviter chevauchements
...
def main():
    ...
    if 'scan_results' in st.session_state:
        ...
        if full_df is None or full_df.empty:
            ...
            # PNG avec matplotlib (simple et sans erreur)
            fig, ax = plt.subplots(figsize=(12, len(df_to_export)*0.3))
            ax.axis('tight')
            ax.axis('off')
            table = ax.table(cellText=df_to_export.values, colLabels=df_to_export.columns, cellLoc='center', loc='center')
            ...
            buf.seek(0)
            st.download_button("🖼️ Télécharger en PNG", buf, f"choch_signaux_{timestamp}.png", "image/png")

            # PDF avec reportlab (texte sélectionnable, sans chevauchement)
            pdf_buffer = create_pdf(df_to_export)
            st.download_button("📄 Télécharger en PDF", pdf_buffer, f"choch_signaux_{timestamp}.pdf", "application/pdf")
